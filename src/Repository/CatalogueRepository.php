...
class CatalogueRepository extends ServiceEntityRepository
{
    ...
    */
    public function OrderByNameDQL()
    {
        $em = $this->getEntityManager();
        $query = $em->createQuery('select s from App\Entity\Catalogue s order by s.nom ASC');
        return $query->getResult();
    }
    public function findCatalogueByNom($nom)
    {
        return $this->createQueryBuilder('catalogue')
            ->where('catalogue.nom LIKE :nom')
